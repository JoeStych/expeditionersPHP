...
// Plant Life discovered by the user
$sql = "SELECT p.* FROM PlantLife p JOIN Expeditioners ex ON p.discovered_by = ex.id WHERE ex.username = '".$_SESSION['username']."' ORDER BY p.name LIMIT $start, $limit";
$result = $conn->query($sql);
$userPlantLife = array();
...
// All Plant Life
$sql = "SELECT p.* FROM PlantLife p ORDER BY p.name LIMIT $start, $limit";
$result = $conn->query($sql);
$allPlantLife = array();
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
...
// Pagination links
$sql = "SELECT COUNT(*) as total FROM PlantLife";
$result = $conn->query($sql);
...
<html>
<head>
    <title>Plant Life Discoveries</title>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="col-sm-9 ml-sm-auto col-md-10 pt-3 main-content">
                <h1>Plant Life Discoveries</h1>

                <h2>Plant Life Discovered by You</h2>
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php if (count($userPlantLife) > 0) { ?>
                            <?php foreach ($userPlantLife as $plantLife) { ?>
                                <tr>
                                    ...
                                    <td><a href="e_view_plantlife_details.php?id=<?php echo $plantLife['id']; ?>" class="btn btn-info">View More</a></td>
                                </tr>
                            <?php } ?>
                        <?php } else { ?>
                            <tr>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php foreach ($allPlantLife as $plantLife) { ?>
                            <tr>
                                <td><?php echo $plantLife['name']; ?></td>
                                <td><?php echo $plantLife['species']; ?></td>
                                <td><?php echo $plantLife['discovery_date']; ?></td>
                                <td><?php echo $plantLife['discovery_location']; ?></td>
                                <td><a href="e_view_plantlife_details.php?id=<?php echo $plantLife['id']; ?>&page=<?php echo $page; ?>" class="btn btn-info">View More</a></td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
